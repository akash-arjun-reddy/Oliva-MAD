...
from utils.twilio_utils import send_otp_sms
...
class AuthService:
    ...
    def send_otp(self, data: SendOTPRequest) -> dict:
        ...
        self.db.query(OTP).filter(OTP.contact_number == data.contact_number, OTP.is_used == False).update({OTP.is_used: True})
        otp = OTP(contact_number=data.contact_number, otp_code=otp_code)
        self.db.add(otp)
        self.db.commit()
    ...
    def signup_with_phone(self, data: SignupWithPhoneRequest) -> dict:
        ...
        if not otp:
            raise HTTPException(status_code=400, detail="Invalid or expired OTP")
        otp.is_used = True
        existing_user = self.db.query(User).filter(User.contact_number == data.contact_number).first()
        if existing_user:
        ...
        new_user = User(
            ...
            email=f"{data.contact_number}@phone.local",
